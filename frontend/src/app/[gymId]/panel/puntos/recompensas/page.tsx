        ...
        rewardsQuery.isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {rewards.map(reward => (
              <div
                key={reward.id}
                className={`rounded-2xl border bg-white overflow-hidden shadow-sm transition-all ${reward.is_active ? 'border-slate-200' : 'border-slate-100 opacity-60'}`}
              >
                {/* Image banner */}
                <div
                ...
                >
                  ...
                  {!reward.is_active && (
                    <span className="absolute top-2.5 right-2.5 text-[10px] font-bold text-slate-600 bg-white/95 px-2.5 py-1 rounded-full shadow-sm">
                      Inactiva
                    </span>
                  )}
                </div>
                ...
                {/* Content */}
                <div className="p-4">
                  <p className="font-semibold text-slate-900 truncate leading-snug">{reward.name}</p>
                  {reward.description && (
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-sm font-bold text-slate-900">{reward.points_cost}</span>
                      <span className="text-xs text-slate-400 ml-0.5">pts</span>
                    </div>
                    <div className="flex items-center gap-1 text-xs text-slate-400">
                      <Package className="w-3 h-3" />
                      {reward.available_stock === null ? 'Stock ilimitado' : `${reward.available_stock} disponibles`}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 border border-slate-200 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50 active:scale-95 transition-all"
                    >
                      <Pencil className="w-3.5 h-3.5" /> Editar
                    </button>
                    <button
                      onClick={() => { if (confirm('¿Eliminar esta recompensa?')) deleteMutation.mutate(reward.id) }}
                      className="flex items-center justify-center w-9 h-9 border border-rose-100 rounded-xl text-rose-400 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 active:scale-95 transition-all"
                    >
