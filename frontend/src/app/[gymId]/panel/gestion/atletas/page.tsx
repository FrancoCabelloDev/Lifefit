...
export default function AthletesPage({ params }: { params: Promise<{ gymId: string }> }) {
  ...
  return (
    <div className="space-y-8 font-inter">
      ...
      {assignModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setAssignModalOpen(false)}>
          <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-md w-full mx-4" onClick={e => e.stopPropagation()}>
            <h3 className="font-bold text-lg text-slate-900 mb-4">Asignar Coach</h3>
            ...
            <div className="flex justify-end gap-2">
                ...
                onClick={async () => {
                  ...
                  try {
                    for (const athleteId of selectedIds) {
                      await api.post("/api/gyms/coach-assignments/", { coach: selectedCoach, athlete: athleteId })
                    }
              ...
              >
                {isAssigning ? <Loader2 className="w-4 h-4 animate-spin" /> : `Asignar a ${selectedIds.size} atletas`}
              </Button>
            </div>
          </div>
        </div>
      ...
      {nutriAssignModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setNutriAssignModalOpen(false)}>
          <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-md w-full mx-4" onClick={e => e.stopPropagation()}>
            <h3 className="font-bold text-lg text-slate-900 mb-4">Asignar Nutricionista</h3>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setNutriAssignModalOpen(false)}>Cancelar</Button>
              <Button
                className="bg-emerald-600 hover:bg-emerald-700"
                ...
                onClick={async () => {
                  ...
                  try {
                    for (const athleteId of selectedIds) {
                      await api.post("/api/gyms/nutritionist-assignments/", { nutritionist: selectedNutritionist, athlete: athleteId })
                    }
                    await fetchNutritionAssignments()
                    setSelectedIds(new Set())
                    setNutriAssignModalOpen(false)
                  } catch (e) { console.error(e) }
                  setIsAssigningNutri(false)
              ...
              >
                {isAssigningNutri ? <Loader2 className="w-4 h-4 animate-spin" /> : `Asignar a ${selectedIds.size} atletas`}
              </Button>
            </div>
          </div>
        </div>
