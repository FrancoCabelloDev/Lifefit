...
function Checkout({ gym, plan, onBack }: {
  ...
  const handleGoogle = async () => {
    ...
    try {
      const data = await api.get<{ authorization_url: string }>(
        `/api/accounts/google/login/?gym_slug=${gym.slug}&plan_id=${plan.id}`,
        { authenticated: false }
      )
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirigir al login del gym tras registro exitoso
      window.location.href = `/${gym.slug}/ingresar?registered=1`
    } catch (err: any) {
      setEmailError(err?.message || 'Error al crear la cuenta. Intenta de nuevo.')
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <h3 className="font-bold text-slate-900 text-lg mb-1">Crea tu cuenta</h3>
        <p className="text-sm text-slate-500 mb-5">
          Elige cómo quieres registrarte para continuar al pago.
        </p>

        {/* Opción Google — destacada */}
        <Button
          onClick={handleGoogle}
          disabled={loadingGoogle}
          variant="outline"
          className="w-full h-12 text-base font-medium border-slate-200 hover:border-slate-300 hover:bg-slate-50 mb-3"
        >
          <GoogleIcon />
        ...
        <div className="relative my-4">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-white px-3 text-slate-400">o con tu email</span>
          </div>
        </div>

        {/* Opción email — secundaria, colapsable */}
        {!showEmailForm ? (
          <button
          ...
          <form onSubmit={handleEmailRegister} className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-slate-600 mb-1 block">Nombre</label>
                <Input value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="Ana" required className="h-10" />
              </div>
              <div>
                <label className="text-xs font-medium text-slate-600 mb-1 block">Apellido</label>
                <Input value={lastName} onChange={e => setLastName(e.target.value)} placeholder="García" required className="h-10" />
              </div>
            </div>
            <div>
              <label className="text-xs font-medium text-slate-600 mb-1 block">Email</label>
              <Input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="ana@email.com" required className="h-10" />
            </div>
            <div>
              <label className="text-xs font-medium text-slate-600 mb-1 block">Contraseña</label>
              <Input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Mínimo 8 caracteres" required minLength={8} className="h-10" />
            </div>
            {emailError && (
              <p className="text-xs text-rose-600 bg-rose-50 border border-rose-100 rounded-lg px-3 py-2">{emailError}</p>
            )}
            <Button
              type="submit"
            ...
            >
              {loadingEmail ? 'Creando cuenta...' : 'Crear cuenta y continuar'}
            </Button>
            <button type="button" onClick={() => setShowEmailForm(false)} className="w-full text-xs text-slate-400 hover:text-slate-600 py-1">
              Cancelar
            </button>
          </form>
        )}

        <div className="flex items-center gap-2 text-xs text-slate-400 mt-5 pt-4 border-t border-slate-50">
